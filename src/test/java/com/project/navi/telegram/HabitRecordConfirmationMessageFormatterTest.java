...
class HabitRecordConfirmationMessageFormatterTest {
    ...
    void congratulatesByNameForBinaryHabitWithoutRemainingInfo() {
        String message = formatter.confirmationFor(lucas, goodFood, null, 0, false);
    ...
    void showsRemainingInBottlesForWater() {
        ...
        String message = formatter.confirmationFor(lucas, water, 500, 1500, false);
    ...
    void usesSingularBottleWhenOnlyOneRemains() {
        String message = formatter.confirmationFor(lucas, water, 1000, 1000, false);
    ...
    void showsRemainingInUnitForNonWaterCumulativeHabit() {
        String message = formatter.confirmationFor(lucas, study, 40, 140, false);
    ...
    void celebratesWhenGoalIsAlreadyReached() {
        String message = formatter.confirmationFor(lucas, water, 500, 0, false);
    ...
    @Test
    void doesNotWarnAboutSummaryByDefault() {
        String message = formatter.confirmationFor(lucas, water, 500, 1500, false);

        assertThat(message).doesNotContain("resumo");
    }

    @Test
    void warnsWhenRegisteredAfterDailySummaryForCumulativeHabit() {
        String message = formatter.confirmationFor(lucas, water, 500, 1500, true);

        assertThat(message).contains("1500ml");
        assertThat(message.toLowerCase()).contains("resumo");
    }

    @Test
    void warnsWhenRegisteredAfterDailySummaryForBinaryHabit() {
        String message = formatter.confirmationFor(lucas, goodFood, null, 0, true);
